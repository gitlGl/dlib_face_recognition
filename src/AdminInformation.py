...
class AdminInformation(QWidget):
    ...
    def browse(self):
        self.result = ShowLog(self.id_number, ['用户ID', '登录时间', "图片"], "admin",
    ...
    def img_event(self, pos):
        ...
        if action == pop_menu.actions()[0]:
            ...
            img_path = "img_information/admin/{0}/{1}.jpg".format(
                str(self.id_number), str(self.id_number))
            ...
            if path:
                ...
                database.c.execute(
                    "update admin set vector = ? where id_number = {0}".format(
                        self.id_number), (vector, ))

                QMessageBox.information(self, 'Success', '修改成功')

    def root(self):
        result = database.c.execute(
            "select id_number,password from admin ").fetchall()
        show_admin_User = ShowAdminUser(['用户ID', '密码', "图片"], 'admin',
                                        ["id_number", 'password'], result)
        self.Vhlayout.itemAt(1).widget().deleteLater()
