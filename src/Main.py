...
# @pyqtSlot(str)
    ...
    def clear_qlabel2(self):
        self.timer.stop()
        self.qlabel2.clear()
        if self.btn3.isChecked() and self.put_img.work_thread.isRunning(
        ):  #and self.put_img.isRunning()
    ...
    def set_normal_img(self, list):
        self.qlabel4.setPixmap(QPixmap.fromImage(
            list[0]))  #设置图片，图片跟随ui.qlabel大小缩放
