import gc,multiprocessing,psutil,uuid
...
class Main(Ui):
    def __init__(self):
        ...
        bool_flag = self.aotuLogin() #检查是否自动登录
        if bool_flag == True:#检查是否自动登录
            self.show()
            return
        if bool_flag != False:
            self.login_ui.emitsingal.connect(self.showParent)
            self.login_ui.show()
            QMessageBox.critical(self, '警告', bool_flag)
            return
    ...
    def posMenu(self,pos):
        ...
        if action == pop_menu.actions()[1]:
            ...
            self.hide()
            self.login_ui = LoginUi()
            self.login_ui.auto_login.setChecked(False)
            self.login_ui.emitsingal.connect(self.showParent)
            self.login_ui.show()
