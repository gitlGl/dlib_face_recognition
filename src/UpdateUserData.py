...
class UpdateUserData(QDialog):
    ...
    def delete(self, id):
        ...
        database.delete(id)
        database.c.execute(
            "delete from student_log_time where id_number = {0}".format(id))
    ...
    def update(self, id):
        ...
        if not os.path.exists(old_path):  #判断是否存在文件夹如果不存在则创建为文件夹
            ...
            os.rename(
                "img_information/student/{0}/{1}.jpg".format(str(id), str(id)),
                "img_information/student/{0}/{1}.jpg".format(
        ...
        if self.path == None:  #图片可以为不变更
            ...
            if (password != self.information["password"]):
                ...
                password = MyMd5().create_md5(password, salt)
                database.c.execute(
                    "update student set id_number= ?,user_name = ?,gender = ? ,vector = ?,password = ?,img_path = ? ,salt = ? where id_number = {0}"
                    .format(id),
                    (id_number, user_name, gender, vector, password,
                     "img_information/student/{0}/log".format(id_number),
                     salt))
            else:
                database.c.execute(
                    "update student set id_number= ?,user_name = ?,gender = ? ,vector = ?,img_path = ?  where id_number = {0}"
                    .format(id),
