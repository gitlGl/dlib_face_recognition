...
class ShowLog(QDialog):
    def __init__(self, id_number, str_list_column, table, list_cloumn):
        ...
        self.page = Page(self.table + "_log_time",
                         self.list_cloumn,
        ...
        self.tableWidget.setEditTriggers(
            QtWidgets.QAbstractItemView.NoEditTriggers)  #禁止用户编辑单元格
    ...
    def set_information(self):
        ...
        for i in self.information:
            ...
            imag_path = "img_information/{0}/{1}/log/{2}.jpg".format(
                self.table, i["id_number"], i["log_time"])
    ...
    def on_tableWidget_cellDoubleClicked(
            self, row,
            column):  #双击槽函数 self.tableWidget.cellDoubleClicked.connect()
        imag_path = "img_information/{0}/{1}/log/{2}.jpg".format(
            self.table, str(self.information[row]["id_number"]),
            str(self.information[row]["log_time"]))
        ...
        if action == delete_event:
            ...
            if self.table == "student":
                item = database.c.execute(
                    "SELECT  cout,id_number from {0} where id_number = '{1}'".
                    format(self.table, str(
                        self.information[row]["id_number"]))).fetchall()[
                            0]  # 取出返回所有数据，fetchall返回类型是[()]
                if item["cout"] is not None:
                    database.c.execute(
                        "UPDATE {0} SET cout = {1} WHERE id_number = {2}".
                        format(self.table, item["cout"] - 1,
                               item["id_number"]))
