...
class studentlog():
    ...
    def insert_time(self):
      ...
      VALUES (?, ?,?)", (self.item["id_number"], self.item["gender"],
    ...
    def insert_img(self, img):
        ...
        if not os.path.exists(path):  # 判断是否存在文件夹如果不存在则创建为文件夹
            os.makedirs(path)
        cv2.imwrite(path + "/" + self.get_time() + ".jpg", img)
    ...
    def insert_cout(self):
        if self.item["cout"] == None:
            cout = 1
            database.c.execute(
                "UPDATE student SET cout = {0} WHERE id_number = {1}".format(
                    cout, self.item["id_number"]), )
...
class adminlog():
    ...
    def insert_time(self):
      ...
      VALUES (?,?)", (self.item["id_number"],
                      datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")))

    def insert_img(self, img):

        path = "img_information/" + "admin/" + str(
            self.item["id_number"]) + "/log"
        if not os.path.exists(path):  # 判断是否存在文件夹如果不存在则创建为文件夹
            os.makedirs(path)
        cv2.imwrite(path + "/" + self.get_time() + ".jpg", img)
