...
class ShowData(QWidget):
    ...
    def show_search_result(self):
        ...
        result = database.c.execute(
            "select id_number,user_name,gender,password from student where id_number = {}"
            .format(id_number)).fetchall()
        if len(result) == 0:
            QMessageBox.critical(self, 'Wrong', '用户不存在')
            return

        result = ShowStudentUser(
            ['学号', '姓名', '性别', '性别', "图片"], "student",
            ["id_number", "user_name", "gender", "password"], result)
        self.Vhlayout.itemAt(1).widget().deleteLater()
    ...
    def browse(self):
        ...
        result = ShowStudentUser(
            ['学号', '姓名', '性别', '密码', "图片"], "student",
            ["id_number", "user_name", "gender", "password"])
        self.Vhlayout.itemAt(1).widget().deleteLater()
        self.Vhlayout.addWidget(result)
        return
    ...
    def analyze_data(self):
        ...
        if days < 1:
            ...
            datatabel, data_title, number = self.get_data(int(temdays / 7),
                                                          7)  #计算x轴步长
            self.view = ChartView(datatabel, data_title, number)
            self.Vhlayout.addWidget(self.view)
        elif days >= 60 and days < 365:
            datatabel, data_title, number = self.get_data(
                int(temdays / 30), 30)
            self.view = ChartView(datatabel, data_title, number)
            self.Vhlayout.addWidget(self.view)
        elif days >= 365:
    ...
    def get_data_(self):
        ...
        for i in timestr:
            reuslt = database.c.execute(
                sql.format(
                    self.DateEdit1.date().toPyDate().strftime("%Y-%m-%d") + i,
                    self.DateEdit1.date().toPyDate().strftime("%Y-%m-%d") +
                    i)).fetchall()
            #print(type(result))
    ...
    def get_data(self, days, step):
        ...
        if days >= 0:
            ...
            for k in range(abs(days) + 1):
                result = database.c.execute(
                    sql.format(
                        self.DateEdit1.date().addDays(
                            step_).toPyDate().strftime("%Y-%m-%d"),
                        self.DateEdit1.date().addDays(
                            step_ +
                            step).toPyDate().strftime("%Y-%m-%d"))).fetchall()
        ...
        if days >= 0:
            ...
            data_title.append(
                self.DateEdit1.date().toPyDate().strftime("%Y-%m-%d"))
            for i in range(days):
                data_title.append(self.DateEdit1.date().addDays(
                    step_).toPyDate().strftime("%Y-%m-%d"))
                step_ = step_ + step
        else:
            step_ = step
            data_title.append(
                self.DateEdit2.date().toPyDate().strftime("%Y-%m-%d"))
            for i in range(abs(days)):
                data_title.append(self.DateEdit2.date().addDays(
                    step_).toPyDate().strftime("%Y-%m-%d"))
                step_ = step_ + step
