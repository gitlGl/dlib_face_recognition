    ...
    def run(num,data,lock = None):
        ...
        for index, item in enumerate(data,start=2):
                ...
                if dic_data is not None:
                    id_number = dic_data['id_number']
                    vector = dic_data.pop("vector")
                    vector_data.append({'idNumber':id_number,"vector":vector})
        ...
        for row ,dic_data in user_data.items():
            ...
            try:
                database.execute(
                    f"INSERT INTO student (id_number,user_name,gender,password ,salt) \
            VALUES ({PH},{PH}, {PH}, {PH} , {PH})",tuple_data)
