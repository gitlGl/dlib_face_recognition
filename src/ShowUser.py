...
class ShowUser(QWidget):
    def __init__(self, str_list_column, table, list_cloumn, information=None):
        ...
        self.tableWidget.setEditTriggers(
            QAbstractItemView.NoEditTriggers)  #禁止用户编辑单元格
        self.tableWidget.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch)  #表示均匀拉直表头
        ...
        self.tableWidget.customContextMenuRequested[QPoint].connect(
            self.context_menu)  #菜单右键槽函数
        self.tableWidget.cellDoubleClicked.connect(
            self.on_tableWidget_cellDoubleClicked)
...
class ShowStudentUser(ShowUser):
    ...
    def on_tableWidget_cellDoubleClicked(
            self, row):  #双击槽函数 self.tableWidget.cellDoubleClicked.connect()
        print(row)
        ...
        self.tableWidget.item(row, 4).setIcon(
            QIcon("img_information/{0}/{1}/{2}.jpg".format(
                self.table, self.information[row]["id_number"],
                self.information[row]["id_number"])))  #获取图片路径)
    ...
    def context_menu(self, pos):
        ...
        row = item.row()
        update_data = UpdateUserData(self.information[row])
        action = pop_menu.exec_(
            self.tableWidget.mapToGlobal(pos))  #显示菜单列表，pos为菜单栏坐标位置
        if action == change_new_event:
            ...
            self.tableWidget.item(row, 3).setText(password)
            self.tableWidget.item(row, 4).setIcon(
                QIcon("img_information/{0}/{2}/{2}.jpg".format(
                    self.table, self.information[row]["id_number"],
                    self.information[row]["id_number"])))  #获取图片路径)
            return

        if action == delete_event:
            r = QMessageBox.warning(self, "注意", "删除可不能恢复了哦！",
...
class ShowAdminUser(ShowUser):
    ...
    def on_tableWidget_cellDoubleClicked(
        ...
        self.tableWidget.item(row, 2).setIcon(
            QIcon("img_information/admin/{0}/{1}.jpg".format(
